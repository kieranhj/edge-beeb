...
# Flags come from the environment: RELEASE, MUSIC_AKL, GFX_CPC, each 0 or 1.
set -e
R=${RELEASE:-0}; A=${MUSIC_AKL:-0}; C=${GFX_CPC:-0}
...
"$BEEB" -i src/main.asm -do build/$STEM-RAW.SSD -opt 3 -title "$TITLE" \
    -D RELEASE=$R -D MUSIC_AKL=$A -D GFX_CPC=$C -v > build/$STEM.lst
