...
**`RELEASE` and `MUSIC_AKL` are beebasm command-line symbols and every build passes both**
(`-D RELEASE=0` or `1`, `-D MUSIC_AKL=0` or `1`). beebasm has no `IFDEF` and refuses a symbol
defined twice, so `main.asm` cannot carry a default; a bare invocation must pass them too:

```
..\..\Bin\beebasm.exe -i src\main.asm -do build\EDGE-RAW.SSD -opt 3 -D RELEASE=0 -D MUSIC_AKL=0 -v
