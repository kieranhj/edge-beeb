...
## Memory (Layer 1 build; take live figures from the listing)
...
| `&0E00-&13C0` | code |
| `&13C1-&1813` | tables, two 126-byte sprite stashes, nibble tables for the 2019 plotter |
...
Banks are loaded by `load_bank` (OSFILE to `&4000`, copied up to `&8000`) at boot, before the
mode is set. **OSFILE overwrites its parameter block's addresses after a load**, so `load_bank`
