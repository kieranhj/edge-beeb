...
| `bank0.asm` | the SWRAM data bank, plus the run-once and out-of-room code: `setup_display`, `clear_play`, `panel_init`, `title_page`, `pause_check`, `comp_mess`, `finale_tick`, the frame meter |
| `bank1.asm`, `bank2.asm` | the two SWRAM sprite banks, one per pixel shift |
| `bank3.asm` | compiled sprite bodies, and the titles' font, credits and text plotter |
...
`src/data/` is generated by `tools/export_tiles.py`, `tools/export_sprites.py` and
`tools/export_waves.py` from `data/`
