...
| Music | The CPC port's Arkos song through `SongToYm`, `ym2sn` and `vgipacker` to a `.vgi` (the `-Akl` build instead replays the tracker data directly, and is the only one with the CPC's SECOND tune, the finale's); `lib/vgiplayer.asm` in HAZEL decodes one byte per register stream per field from `rupt_vsync`. **The whole 349 seconds ships, spread over FOUR regions of memory** - a `.vgi` is eleven independent streams and only each stream has to be contiguous (decision 48). `tools/export_music.py` places them and `tools/verify_vgi.py` proves the placement byte for byte against a jsbeeb capture |
...
| Artwork | The characters, the sprites, **the status panel, the HUD font, the title page's font** and **the palette** are read from PNGs in `assets/art/` (Layer 8, decisions 58-62), seeded from the C64 conversion so the disc is unchanged until the artist starts work. Five sheets at 2:1 - `chars-bbc.png` 128x128, `sprites-bbc.png` 192x336, `panel-bbc.png` 320x40 (the bar as a PICTURE, cut into 200 cells on export), `hud-bbc.png` 128x8 (blank, digits 0-9, the life marker's two halves) and `titlefont-bbc.png` 128x16 (blank, A-Z, `! . , - ?`; the memorial draws through it too). **A `Sheet` carries the logical colours it may use**: sprites may not use 0 (the transparency key, so black is 8) and the title font may use ONLY 0, 12, 14, 15 - the same RGB as 4, 6, 7, so without that rule a painted font would look right and fade the panel with the credits (decisions 53, 62; flagged for revisiting, see `docs/layer-9e-credits.md`) - plus `palette.png`, sixteen entries, generated to `src/data/palette.asm`. **A panel cell, a HUD glyph and a character are the same 4x8 shape**, which is why one `Sheet` and one `pack_cell` serve all four. **Eighteen panel cells are overwritten by the HUD** (`hud_cell_lo/hi`); `check_hud_cells` warns about ink painted there - a warning, because the C64's bar leaves them blank and the Amstrad's does not. **Characters are the paintable surface; tiles and the map are index tables and stay the C64's**, so `render_bbc.py tiles`/`map` regenerate the assembled views. Grey `96,96,96` is see-through (sprites only), orange `255,128,0` is 'not drawn yet' and falls back to the mechanical conversion. **The level itself can be painted too** (decision 68): `tools/paint_map.py export` writes any range of tile columns as one PNG and `import` reads it back into `chars-bbc.png`, reconciling the ~40 instances of each character by vote - one changed instance wins, two changed differently is refused with both map columns. It moves character bitmaps ONLY; the tile table, the map and the character numbers stay the C64's, so the 256-character budget cannot be overrun. `export --c64` / `--cpc` draw the same level with the ORIGINALS' charsets at their own sixteen colours, to `tools/output/`, as the artist's reference - the C64 one is byte for byte `reference/map-c64.png`. `--grid` adds a transparent guide LAYER beside the picture - character grid, tile grid, tile column numbers - which `import` refuses, transparency being proof it is not a canvas. **`tools/art_grid.py` writes the same guide for the five sheets** (`assets/art/*-grid.png`, gitignored): cell boundaries with every fourth brighter, and the 2 x 1 fat pixel pairs - **numbered only where the cell has room to spare**, which is the sprite sheet alone; an 8-pixel character cell gets the every-fourth ruler instead. `tools/art/guide.py` is the shared font and rules. `tools/art/`, `tools/seed_art.py`, `tools/validate_art.py`, `tools/export_palette.py`, `tools/paint_map.py`, `docs/layer-8-art-pipeline.md` |
...
| Controls | **Redefinable: CTRL+R on the titles** (decision 71, `docs/layer-9h-keyredef.md`) writes `joy_keys`, five bytes of ZERO PAGE, which is why `ldx joy_keys + JOY_FIRE` is the same two bytes `ldx #KEY_FIRE` was and bank 0 did not grow. Defaults Z/X left/right, K/M up/down, L fire, **SPACE also starts a game from the titles** (decision 54), **CTRL+A** on the TITLES toggles auto-fire, default OFF, popping `AUTO FIRE ON`/`OFF` up centred on credit row 1 for three seconds - the gap the C64's credit spacing leaves, drawn in logicals 4/6/7 so the credit crossfade cannot black it out (decision 73). **`af_latch` in zero page is the VALUE `fire_bullet` writes into `fire_latch`, not a flag**: `AF_OFF` = `&80` is negative and is the C64's latch, cleared only by a release; `AF_ON` = 10 is a countdown in game ticks that the held path decrements, so holding fire is floored at one shot per 10 ticks while a TAP still gets the next shot as early as the bullet slot allows (decision 74 - without it the rate was the bullet's flight, which is 15 ticks at normal range, 6 at the right-hand edge and less still point blank, because a bullet that hits something dies where it hit). **Ten binds where the flight is short and nowhere else**, which is the point: normal range is unchanged and close in is capped. `AF_ON` is KC's number, played not calculated, and is the one to turn. `docs/layer-9i-autofire.md`, **CTRL+P** pause (decision 72; CTRL+P or fire unpauses), ESCAPE abort (only while paused - always-on was built and reverted: it is polled every frame and holding it rebuilt the player's explosion pieces every frame), **CTRL+Q** mute (decision 72). Internal key numbers are **measured** (OSBYTE 121, or `INKEY(-n)` in a BASIC session holding the key), never recalled - Z 97, X 66, K 70, M 101, L 86, P 55, Q 16, SPACE 98, ESCAPE 112, and Layer 9h's thirty-five bindable keys plus CTRL 1. **SHIFT (0) and CTRL (1) are below OSBYTE 121's floor** - the MOS scan will not report keys 0-2 - so those two are measured through INKEY and calibrated on a known key. `*FX229,1` first, or BASIC eats ESCAPE. **CTRL+Q is read in the VSync handler**, so it works on the titles and in the finale as well as in play (decision 39). **The CTRL test goes FIRST in both**, which makes it free: CTRL is up in almost every frame, so the common path is the one keydown the ungated test used to cost. **The tune stops while the game is paused** and Q is not read there at all, there being nothing to mute (decision 43) |
...
has 379 left, so it could not go there). **Both tunes moved up a page in Layer 9i, to `&9200`
and `&A500`**, to make bank 3 room for the auto-fire indicator; AKL data is absolute, so
`tools/export_music_akl.py` carries the same two numbers and both were re-exported, and `music_change` switches to it in `rupt_vsync` the way
the CPC's `ChangeMusic` does. It needs `WIN_TRANSP0..2` set into `t_transp` after `akl_init`,
because AT2's exporter drops position 0's transposition and the finale plays in the wrong key
without them - `tools/export_music_akl.py --check` fails if that ever changes. **It does not sound the same** - the offline chain does
...
| `bank3.asm` | compiled sprite bodies; the titles' font, credits and text plotter, and **the auto-fire indicator's two lines and `title_auto`** (Layer 9i, decision 73 - drawn in logicals 4/6/7 so the credit crossfade cannot touch it, and the one thing in the build that a VGI `-Cpc` bank 3 has no room for); **the memorial's message and `mem_page`** (Layer 9d), which is the half of it that needs the font; the HUD glyphs and `status_decode`; then region A of the tune from `&9100` to the join at `&C000`. Reached from main RAM through `bank3_call` |
