# What each finished layer did

*The detail [`PLAN.md`](../PLAN.md) used to carry, moved here on 2026-09-05 when it stopped being
needed to decide what to do next — which is what PLAN.md's own preamble says should happen to it.*

Nothing here is live state. Each entry links to the layer's own document, which is the real record;
this is the one-paragraph summary of what the layer did and what it cost, in the order the layers
were built. **The figures are as they were when the layer was written and have not been
re-measured since** — take live numbers from the build listing, and the current memory picture from
[`memory-map.md`](memory-map.md) and PLAN.md's table.

## Layer 1 — graphics pipeline A — done

[`layer-1-graphics-pipeline.md`](layer-1-graphics-pipeline.md). Exporters in `tools/`,
committed output in `src/data/`, `render_bbc.py` for desktop checks. Sprite data consumption
deferred to Layer 3.

## Layer 2 — display — done

[`layer-2-display.md`](layer-2-display.md). Two-cycle rupture with the panel at
...
[`layer-3-sprites.md`](layer-3-sprites.md). Eight software sprites in both banks:
...
[`layer-4-player.md`](layer-4-player.md). Movement and bounds, the fire latch, the bullet,
...
[`layer-5-enemies.md`](layer-5-enemies.md). `wave_manager`, `wave_read` and the **201-wave**
...
[`layer-6b-life-cycle.md`](layer-6b-life-cycle.md). `coll_flag` is read: three lives, the
...
[`layer-6c-state-machine.md`](layer-6c-state-machine.md). `master_loop`: titles →
...
[`layer-6d-hud.md`](layer-6d-hud.md). The colour-bar placeholder is gone. The C64's status
...
[`layer-6e-titles.md`](layer-6e-titles.md). The titles move: a six-row zoom scroller across
...
[`layer-7-music.md`](layer-7-music.md). `tools/export_music.py` runs EDGEA.SKS through
...
## The frame budget, as it was measured

*The per-layer figures PLAN.md carried. They are the measurements each layer was signed off on and
have not been re-taken since; [`performance.md`](performance.md) is the fuller record, and
PLAN.md keeps only the summary.*
...
[`layer-9-loader.md`](layer-9-loader.md).
...
[`layer-7-music.md`](layer-7-music.md).

## 9c — the outstanding features, as they were found
...
[`layer-9c-starfield.md`](layer-9c-starfield.md), decisions 50 and 51. Ten stars on
...
[`layer-9c-mega-hero.md`](layer-9c-mega-hero.md).
